...
export class ItemPricePipe implements PipeTransform {
	...
	transform(item: Item, type: 'rent' | 'buy' | 'sell', period?: 'semester' | 'year', numberOfPeriods?: number): any {
		if (type === undefined) {
			return -1;
		}
		...
		switch (type) {
			case 'rent':
				return this.prettyOutput(this.calculatePriceRent(item, period, numberOfPeriods));
			case 'buy':
	...
	private calculatePriceRent(item: Item, periodType: 'semester' | 'year', numberOfPeriods: number): number {
