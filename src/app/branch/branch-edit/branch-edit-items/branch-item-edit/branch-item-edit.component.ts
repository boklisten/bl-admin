...
export class BranchItemEditComponent implements OnInit {
	...
	onUpdate() {
		this._branchItemService.update(this.branchItem.id, this.branchItem).then((updatedBranchItem: BranchItem) => {
			this.branchItem = updatedBranchItem;
		}).catch((updateBranchItemError) => {
			console.log('branchItemEditComponent: could not update branchItem', updateBranchItemError);
		});
	}
